...
def nonregularized_derivative(x, y, data, shape, order):
    ...
    """
    Computes the non-regularized directional derivatives in the Fourier domain in the x-, y-, and z-directions using equation 3 of the paper.
...
def regularized_derivative(x, y, data, shape, order, alpha):
    ...
    """
    Computes the regularized directional derivatives in the Fourier domain in the x-, y-, and z-directions using equation 5 of the paper.
    ...
    * dx, dy, dz: 1D-array
    ...
    # Calculates the derivatives in the Fourier domain
    derivx_fft = np.fft.fft2(padded) * (gamma_x ** order)
    derivy_fft = np.fft.fft2(padded) * (gamma_y ** order)
    derivz_fft = np.fft.fft2(padded) * (gamma_z ** order)
